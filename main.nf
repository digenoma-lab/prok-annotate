...
workflow {
	...
	if ( params.run_eggnog ) {
		...
		eggnog_mapper(genomes_ch, eggnog_db)
	...
	if ( params.run_kofamscan ) {
		if (params.kofamscan_db == 'none') {
			...
			kofamscan_db = file(params.kofamscan_db, type: 'dir', checkIfExists: true)
		}
	...
	if ( params.run_anvio_kofam ) {
		...
		merge_anvio(anvio_kofam.out.hits.collect(), anvio_kofam.out.modules.collect())
	}
}
